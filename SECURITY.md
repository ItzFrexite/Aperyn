...
The 11435 proxy is not covered by WebUI login in 1.27.0; restrict it by firewall. The helper binds to loopback, uses a host-persistent token, exposes fixed routes, and allow-lists every setting. The WebUI has no Docker socket, privileged mode, writable host root, arbitrary shell, or generic systemd control.
