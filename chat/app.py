    ...
    def generate():
        ...
        try:
            ...
            if provider != 'ollama':
                ...
                external_status = 200
                return
            upstream = requests.post(f'{OLLAMA_API}/api/chat', json=payload, stream=True, timeout=3600, headers={'X-Ollama-Control-Client':'WebUI Chat','User-Agent':'Aperyn-WebUI/1.27.6'})
            if not upstream.ok:
